...
public class UserNameLookupController extends AuthenticationBase {
    ...
    public String userNameLookupForm(@RequestParam("email_address") String emailAddr, @RequestParam("tenant") String tenant,
        ...
        if (Strings.isNullOrEmpty(emailAddr)) {
            ...
            if (EmailValidator.isValid(emailAddr)) {
                try {
                  UserInfo info = authService.findUserByEmailAddr(emailAddr,tenant);
                  if (info != null) {
