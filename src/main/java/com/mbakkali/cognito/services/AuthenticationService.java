...
public class AuthenticationService implements AuthenticationInterface, CognitoResources {
    ...
    protected AWSCognitoIdentityProvider getAmazonCognitoIdentityClient() {
        AWSCredentials credentials = getCredentials(iamCognitoPowerUserID, iamCognitoPowerUserKey);
     ...
     * @param location The users tenant (e.g., Bonaire, Aruba, Curacao)
    ...
    public void createNewUser(final UserInfo userInfo) throws AWSCognitoIdentityProviderException {
        ...
        if (!Strings.isNullOrEmpty(emailAddr) && !Strings.isNullOrEmpty(tenant)) {
            // There should be no user with this email address, so info should be null
            UserInfo info = findUserByEmailAddr(emailAddr, tenant);
            if (info == null) {
                AdminCreateUserRequest cognitoRequest = new AdminCreateUserRequest()
                        .withUserPoolId(poolID)
                        .withTemporaryPassword("Aszqaszq1!")
                        .withDesiredDeliveryMediums(ImmutableList.of("EMAIL"))
                ...
                PasswordRequest passwordRequest = new PasswordRequest(userInfo.getUserName(),"Aszqaszq1!",userInfo.getPassword());
                changeFromTemporaryPassword(passwordRequest);
            } else {
    ...
    public UserInfo getUserInfo(final String userName) throws AWSCognitoIdentityProviderException {
        ...
        for (AttributeType attr : userAttributes) {
            ...
            } else if (attr.getName().equals(TENANT)) {
                tenant = attr.getValue();
            }
        }
        UserInfo info = null;
        if (rsltUserName != null && emailAddr != null && tenant != null) {
            info = new UserInfo(rsltUserName, emailAddr, tenant);
        }
        return info;
    } // getUserInfo
     ...
     */
    @Override
    public UserInfo findUserByEmailAddr(String email, String tenant) throws AWSCognitoIdentityProviderException {
        UserInfo info = null;
        if (!Strings.isNullOrEmpty(email)) {
            final String emailQuery = "email=\"" + email + "\"";
            ListUsersRequest usersRequest = new ListUsersRequest()
                    .withUserPoolId(poolID)
                    .withAttributesToGet(EMAIL, TENANT)
            ...
            //Custom attributes not filterable
            //https://docs.aws.amazon.com/sdkforruby/api/Aws/CognitoIdentityProvider/Types/ListUsersRequest.html
            users = filterUserListByTenant(tenant, users);
                    ...
                    if (attributes != null) {
                        for (AttributeType attr : attributes) {
                            if (attr.getName().equals(EMAIL)) {
                                ...
                                aTenant = attr.getValue();
                            }
                        }
                        if (userName != null && emailAddr != null && aTenant != null) {
                            info = new UserInfo(userName, emailAddr, aTenant);
                        }
                    }
                } else {
    ...
    private List<UserType> filterUserListByTenant(String tenant, List<UserType> users) {
        List<UserType> collect = users.stream().filter(userType -> {
            for (AttributeType attribute : userType.getAttributes()) {
                if (attribute.getName().equals(TENANT)) {
                    attribute.getValue().equals(tenant);
                    return false;
                }
            }
            return true;
